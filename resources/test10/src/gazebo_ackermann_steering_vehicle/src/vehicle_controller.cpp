...
VehicleController::VehicleController(const double timer_period, const double timeout_duration) :
  ...
  max_steering_angular_velocity_{1.5708},
  ...
  wheel_steering_angle_{0.0, 0.0}
{
  ...
  declare_parameter<double>("max_steering_angular_velocity", 1.5708);
  ...
  get_parameter("max_steering_angular_velocity", max_steering_angular_velocity_);
...
void VehicleController::timer_callback()
{
  ...
  // Rate-limit steering toward target
  const double max_step = max_steering_angular_velocity_ * timer_period_;
  const double error = target_steering_angle_ - steering_angle_;
  if (std::abs(error) <= max_step) {
    steering_angle_ = target_steering_angle_;
  } else {
    steering_angle_ += (error > 0.0 ? max_step : -max_step);
  }
...
void VehicleController::steering_angle_callback(const std_msgs::msg::Float64::SharedPtr msg)
{
  ...
  // Store as target; timer_callback ramps steering_angle_ toward it
  if (msg->data > max_steering_angle_) {
    target_steering_angle_ = max_steering_angle_;
  } else if (msg->data < -max_steering_angle_) {
    target_steering_angle_ = -max_steering_angle_;
  } else {
    target_steering_angle_ = msg->data;
  }
}
